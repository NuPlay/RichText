...
struct RichTextAllTests {
    ...
    struct NewFeaturesTests {
        ...
        @Test("Background color CSS generation", arguments: [
            ...
            (BackgroundColor.hex("#FF0000"), "#FF0000"),
            (BackgroundColor.hex("ABC"), "#ABC"),
            (BackgroundColor.hex("rgba(0, 123, 255, 0.05)"), "rgba(0, 123, 255, 0.05)"),
            (BackgroundColor.hex("red"), "red"),
    ...
    struct BackwardCompatibilityTests {
        ...
        func deprecatedBackgroundColorMethods() {
            ...
            let deprecatedRGBA = RichText(html: "<p>Test</p>")
                .backgroundColor("rgba(0, 123, 255, 0.05)")

            #expect(deprecatedRGBA.configuration.backgroundColor.cssValue == "rgba(0, 123, 255, 0.05)")
        }
    ...
    struct ColorSetEqualityTests {
        ...
        @Test("Platform colors convert to valid hex values")
        func platformColorHexConversion() {
            #if canImport(UIKit)
            let colorSet = ColorSet(light: UIColor.black, dark: UIColor.white)
            #else
            let colorSet = ColorSet(light: NSColor.black, dark: NSColor.white)
            #endif

            #expect(colorSet.isValid)
            #expect(colorSet.rawValue(true) == "000000")
            #expect(colorSet.rawValue(false) == "FFFFFF")
        }
    }
}
