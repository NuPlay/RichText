...
extension RichText {
    ...
    public func backgroundColor(_ backgroundColor: String?) -> RichText {
        ...
        if let bg = backgroundColor {
            let normalizedBackground = bg.trimmingCharacters(in: .whitespacesAndNewlines)

            if normalizedBackground.lowercased() == "transparent" {
                result.configuration.backgroundColor = .transparent
            } else if normalizedBackground.lowercased() == "inherit" {
                result.configuration.backgroundColor = .system
            } else {
                result.configuration.backgroundColor = .hex(normalizedBackground)
            }
        } else {
            result.configuration.backgroundColor = .transparent
        }
